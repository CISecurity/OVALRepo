...
def main():
    ...
    # collect the file paths for all ids changed and their upstream ids
    file_paths = elements_index.get_paths_from_ids(affected_oval_ids)

    # add each OVAL definition to generator and write to file
    message('info',"Generating OVAL definition file '{0}' with {1} elements".format(args['outfile'], len(affected_oval_ids)), verbose)
